...
```
01-platform/         — Продуктовая архитектура (канон: product-architecture.md), ограничения, команда
...
03-product/onebi-dev/ — Техническая документация OneBi бэкенда (ADR, API spec, DDD guide) — из wiki.cas.ai
06-marketing-site/   — Маркетинговый сайт cas.ai (лендинги, воронки, A/B тесты)
assets/              — Картинки (mockups, конкурентный анализ)
99-archive/          — Устаревшие доки (зеркальная структура; НЕ источник правды, не читать как факт)
```
...
| Задача | Файлы |
|--------|-------|
| Понять продукт | `01-platform/product-architecture.md` (канон, 4 слоя) → `03-product/cabinet/overview.md` |
| Продуктовая архитектура (канон) | `01-platform/product-architecture.md` |
| Карта контекстов (границы, стыки, владельцы) | `01-platform/context-map.md` |
| Пользователи платформы (типы, роли, RBAC) | `01-platform/platform-users.md` |
| Ролевые АРМ / RBAC (Слой 4) | `03-product/cabinet/am-cabinet.md` |
...
| Словарь absolute-метрик (base) | `04-bi/absolute-metrics-dictionary.md` |
| User stories | `03-product/user-stories.md` |
| R&D гипотезы | `03-product/rnd-hypotheses.md` |
| Онбординг | `03-product/onboarding-flow.md` |
| Стратегия и фазы | `03-product/product-strategy-1pager.md` |
...
| Архитектура бэкенда OneBi | `03-product/onebi-dev/` (ADR, DDD guide, API spec) |
...
- `99-archive/` — только исторический контекст для людей; НИКОГДА не использовать как источник правды.
- Кабинет ≠ OneBI. OneBI = слой аналитики (Слой 2), встроенный в кабинет. Канон — `01-platform/product-architecture.md`.
