...
> **Дата:** 2026-03-16 (обновлено 2026-03-18)
...
> **Источники:** конкурентный анализ (AppLovin, Unity, Appodeal, DT, Yodo1, Moloco, Voodoo), встреча с Horyk 17.03.2026 (валидация Internal Ops)
...
- Единый дашборд `dash.applovin.com`: MAX (supply) + Growth (demand) под одним логином. **Уточнение (Horyk 17.03):** на практике в MAX «всё тоже разбросано в разных разделах» — unified dashboard преувеличен
...
|----------|----------------|---------------------|
| AppLovin MAX | Формально да, но разделы разбросаны (Horyk) | 73% ↑ |
...
│
├── CAS Configuration (Internal → Self-service)
│   ├── Waterfall/bidding configuration (по странам!)
│   ├── SDK-конфигурация (связка с зелёным сайтом)
│   ├── A/B testing конфигураций
│   ├── SplitEngine (self-service запуск, registry, auto-significance)
│   ├── Network management
...
├── CAS Configuration (АРМ монетизации — dashboard → зелёный сайт → SDK)
...
## CAS Configuration — третий слой (бывш. Internal Ops)

> **Статус:** Валидировано с Horyk 17.03.2026. Название согласовано: **CAS Configuration**. Backlog = проект в Asana.

Во всех трёх вариантах есть два слоя: **клиентская консоль** (то, что видит паблишер) и **под капотом** (SDK, Exchange, Data). Но есть ещё третий слой — **АРМ команды монетизации**: то, что видят менеджеры CAS, а не клиенты.

Сейчас эту роль выполняет **1С**, и с неё надо мигрировать. Ключевая цитата Горика: *«Я не хочу общаться с зелёным сайтом через 1С. Я ещё относительно могу, моя команда — не может.»*

### Текущая цепочка (проблема)

```
Человек → 1С → Зелёный сайт (remote config) → SDK
                  ▲
          bottleneck:
          нет гибкости по странам,
          нет кастомных сетапов,
          80% тестов Горик правит за людьми
```

### Целевая цепочка

```
Человек → CAS Configuration (dashboard) → Зелёный сайт → SDK
                    │
                    └──→ 1С (по API, только для учёта доходов)
```

Зелёный сайт = remote config для SDK. Он **умеет** по странам. 1С — **не умеет**. Bottleneck = 1С как прокладка.

### Что живёт в CAS Configuration
...
└────────────────────┬────────────────────────────┘
                     │
┌────────────────────┴────────────────────────────┐
│    CAS Configuration (менеджеры CAS видят)      │  ← ЭТОТ СЛОЙ
│                                                 │
│    ├── Network & SDK Config  ★ ПРИОРИТЕТ        │
│    │   ├── Waterfall-конфигурации (по странам!) │
│    │   ├── SDK-конфигурация (связка с зелёным)  │
...
│    ├── Client Onboarding                        │
│    │   ├── Онбординг клиента                    │
│    │   ├── Онбординг приложения (app)           │
│    │   └── Стартовое подключение сетей          │
│    │   ⚡ ~90% автоматизировано ботами (ОК)     │
│    │                                            │
│    ├── Client Operations (остаётся в 1С пока)   │
│    │   ├── Единый реестр клиентов (Задача 1)    │
│    │   ├── HubSpot↔Bitrix↔1C маппинг           │
│    │   ├── Revenue tracking по клиентам         │
│    │   └── Реферальная программа (Задача 2)     │
│    │                                            │
│    ├── Monitoring & Alerts (бэклог, не горит)   │
│    │   ├── Drop detection (Задача 3)            │
│    │   ├── АРМ менеджера монетизации            │
│    │   └── Каскадная система алертов            │
│    │                                            │
│    ├── Reporting & Accounting (остаётся в 1С)   │
│    │   ├── Revenue reports по менеджерам         │
│    │   ├── Billing / сверка                     │
│    │   └── Scorecard для All-Hands              │
│    │                                            │
│    └── GTM Stack (остаётся в 1С)                │
│        ├── CRM (HubSpot + Bitrix)               │
│        ├── Marketing site                       │
│        └── Attribution                          │
└─────────────────────────────────────────────────┘
...
### Что выносим первым (согласовано с Horyk)

**Network & SDK Config** — единственный блок, который нужно выносить из 1С сейчас. Всё остальное может подождать.

Экономический эффект:
- Гибкие тесты **по странам** (сейчас только full countries). Пример: floor 26 не нужен в Индии, разные core mediators для T1 vs T3
- **Больше экспериментов** — быстрее и качественнее, без ручной правки за командой
- Снятие bottleneck: Горик лично правит **80% тестов** за своими людьми
- Возможность обслуживать больше клиентов без масштабирования команды

Что **НЕ трогаем**:
- Онбординг клиентов — **~90% автоматизирован** ботами, работает нормально
- Client Operations, Reporting, GTM — пока живут в 1С, и это ОК
- 1С продолжает получать конфигурацию (через API), т.к. от неё зависит загрузка доходов

### Gaming vs Non-gaming (инсайт Horyk)

Состав портфеля CAS меняется:
- Исторически: **гейминг**. Сейчас: ~$500K non-gaming из ~$2M оборота, растёт (+$300K новых)
- **Non-gaming = 90% revenue от AdMob**, нужна AdMob-медиация как core
- **Gaming = лучше IronSource/AppLovin** как core mediator
- Настройки кардинально отличаются по жанрам → CAS Configuration должен поддерживать **разные профили настроек по типу приложения**

Это важно для тиерной модели: один и тот же клиент может иметь gaming и non-gaming приложения с разными оптимальными конфигурациями.

### R&D — три роли, не путать (уточнение Horyk)

| Кто | Что делает | Отношение к R&D |
|-----|-----------|-----------------|
| **Горик** | Тестирует **готовые** фичи SDK на реальном портфолио (какой core mediator для какого жанра/страны) | R&D: применение |
| **Юра Витюк** | Тестирует **новые** фичи, которых ещё нет в SDK | R&D: эксперименты |
| **Денис Ещенко** | Реализует фичи в SDK по заказу | **Не R&D — исполнитель** |

Горик: *«Юра работает с новыми фичами, я — с готовыми. Ещенко — исполнитель фичей, которые у него заказывают.»*

### Команда Горика — два отдела

1. **Настройки + A/B тесты + общение с сетями** (Алина в подчинении)
2. **Подключение сетей** (онбординг новых клиентов)

### Нейминг: не путать серверы

| Название | Что это | Тип |
|----------|---------|-----|
| **CAS Configuration** | АРМ команды монетизации (dashboard) | Продукт для людей |
| CAS Bidding Server | Серверные биддинги (сейчас эмуляция на клиенте) | Технический компонент |
| CAS Events Server | Сбор ILRD-событий с устройств | Технический компонент |
| Зелёный сайт | Remote config для SDK (SDK берёт конфигурацию отсюда) | Технический компонент |

Slack-канал по теме: `#cas-server-sdk-config`

**Вопрос (остаётся открытым):** куда мигрирует остальное (не config)?
...
- **Гибрид** — reporting/monitoring → OneBI, network config → CAS Configuration, client ops → 1C replacement

Факт: задачи Влада Horyk (#7-14 из xlsx) требуют данных из CAS Events Server, которых в 1С нет. А задачи Смирнова (#1-6) — классическая 1С-автоматизация, но на ClickHouse данных.
...
| A5 | Настройка сетей | Internal (менеджер делает) | 1C, ручная работа. Онбординг ~90% автоматизирован ботами (ОК), но кастомные настройки — через 1С | CAS Configuration |
...
| B5 | Waterfall config | Сейчас = менеджер в 1C → зелёный сайт. Горик правит 80% тестов за командой | **Self-Service НЕТ**. Сначала CAS Configuration (internal), потом self-service | CAS Config → Self-Service |
...
CAS Configuration:
  Network & SDK Config  ░░░░░░░░░░░░░░░░ ███ P1 ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓
  Остальное (от 1С)     ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░ ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓

███ = начало    ▓ = активная работа    ░ = ещё не началось
```

**Приоритет в одном абзаце:** Сначала SDK stable + docs (без этого все три journey ломаются). Потом OneBI Quick View (Entry-клиенты видят хоть что-то). Потом OneBI Reports + Drop Detection (Pro-клиенты получают ценность). **CAS Configuration (Network & SDK Config)** — параллельный трек H1-H2: снимает bottleneck Горика (80% тестов правит за командой), даёт гибкие тесты по странам, не зависит от клиентских продуктов. Self-Service и Publishing — H2+, потому что зависят от предыдущих слоёв.
