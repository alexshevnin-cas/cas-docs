...
> **Контекст модели:** OneBI = **Слой 2** (слой аналитики), встроен в кабинет, статус **Live**.
> Схема «Архитектура (целевая)» ниже = **Слой 1** (Data Platform). «B2B база» = текущий источник;
> хост — platform.cas.ai, b2b.cas.ai — legacy на период перехода. Канон слоёв →
...
Платформа на platform.cas.ai (b2b.cas.ai — legacy на период перехода). Руслан строит новое SPA (Vue 3, шаблон, тёмная тема). Данные — API рекламных сетей, задержка ~1.5 суток.
